...
This repository is meant to be used as a single project, not as standalone scripts.

* Clone or download the entire repository (including all folders/subfolders and R scripts).
* Open the **ORCID-Data-Visualization** folder as your project:  
  - In RStudio: open `ORCID-Data-Visualization.Rproj`  
  - In VS Code: open the `ORCID-Data-Visualization` folder as your workspace.
* All scripts assume the working directory is the project root and use relative paths (e.g., `data/orcid-data.csv`) instead of hardcoded absolute paths.
* You should not need to call `setwd()` in your own code to run these scripts.
